...
function populateProducts() {
    ...
    const productsHtml = products.map((product, i) => {
			return (
                ...
                <div class="row">
				<div class="col-2">
					<img src="${product.image}" width="100%" alt="${product.description}">
				</div>
				<div class="col-2">
					<h2>${product.name}</h2>
					<h3>${product.price}€/h</h3>
					<a href="#" class="add-cart btn"> Marca já </a>
					<h3> Mais informação </h3>
					<p> Entraremos em contacto para agendar as aulas num prazo máximo de 24h. <br>
					As nossas explicações são via Skype, Google Meet ou Zoom. <br> Podem ser explicações individuais ou em grupos de 2 pessoas. <br> Valores mensais. </p>
				</div>
			</div>
					`
			)
